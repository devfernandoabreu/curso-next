...
export default function Category({ products, category }: CategoryProps) {
  ...
  return (
    <div>
      <SEO title={PrimicDom.RichText.asText(category.data.title)} />

      <Link href="/">
        <a>Back to home</a>
      </Link>

      <h1>Category: {PrimicDom.RichText.asText(category.data.title)}</h1>
...
export const getStaticProps: GetStaticProps<CategoryStaticProps> = async (
  context,
) => {
